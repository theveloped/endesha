...
Start-Process -WindowStyle Hidden cmd -WorkingDirectory $root -ArgumentList '/c', 'pixi run python -m wf.hal.arm_sim --cell deploy/cell.dev.yaml --resource r1 --realm sim --zenoh-config deploy/zenoh/driver-router.dev.json5 > deploy\logs\arm_sim.log 2>&1'
Start-Process -WindowStyle Hidden cmd -WorkingDirectory $root -ArgumentList '/c', 'pixi run python -m wf.hal.genicam --cell deploy/cell.dev.yaml --resource cam0 --realm live --zenoh-config deploy/zenoh/driver-router.dev.json5 > deploy\logs\camera.log 2>&1'
Start-Process -WindowStyle Hidden cmd -WorkingDirectory $root -ArgumentList '/c', 'pixi run python -m wf.services.recording.recorder --realm live --out-dir deploy\recordings --zenoh-config deploy/zenoh/driver-router.dev.json5 > deploy\logs\recorder.log 2>&1'
...
Write-Host "stack starting: arm driver + sim arm + camera driver (retries when the camera is off) + recorder (idle until 'wfctl record start') + config service (config/**) + bridge (ws://127.0.0.1:10000) + ui (http://localhost:5173); logs in deploy\logs\. Sim camera runs via 'docker compose -f deploy/compose.yaml --profile sim up' (OSMesa render is Linux-only)."
