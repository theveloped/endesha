"""The ``arm_dio`` provider process: ``python -m wf.hal.arm_dio --cell <realized>
...
def main(argv=None) -> int:
    parser = argparse.ArgumentParser(prog="arm_dio", description=__doc__)
    ...
    core = DioCore(session, args.realm, args.resource, params, ArmDioBackend(params))
    try:
        core.start()
        core.run_forever()
    finally:
